...
| `frames/preview-01.jpg` | 21KB 首屏轻量预览，用于移动端即时可用与加载占位 |
| `frames/small-*.jpg` | 六阶段静态兜底图（移动端视频无法预载时自动切换，交叉渐变） |
| `bgm.m4a` | 背景音乐（循环播放） |
...
- 首屏会先加载 21KB 轻量预览并开放交互，连续形变视频在后台完成后自动升级。
- BGM 仅在用户首次交互后请求，避免与首屏人像资源抢占带宽。
